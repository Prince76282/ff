...
const Nftcartpages = ({ width, height, bgcolor,cardetails }) => {
  ...
  return (
    <div
      ...
      className={clsx(
        `border-2 sm:border-4 z-10  text-white rounded-sm overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 relative`,
        width, 
        height 
      )}
      ...
      {!hovernft ? (
        <>
          ...
          <div className="flex flex-row justify-between items-center  text-white px-3 sm:px-4 py-2">
            <p className="text-sm  md:text-xs font-semibold font-mono">
             {cardetails?.product_name || "NFT Name"}
            </p>
            <span className="text-gray-400 text-xs sm:text-sm font-mono">
              { "#" + cardetails?.product_id||"@nkk01"}
            </span>
          </div>

          {/* NFT Image */}
          <div className="w-52 lg:h-[180px] sm:h-[180px] md:h-[180px] h-auto overflow-hidden shadow-lg  p-2">
            <img
              src={ cardetails?.poster_image||dhoni}
              alt="NFT"
          ...
          >
            ...
            <div className="flex flex-row justify-between items-center mt-2 flex-wrap">
              <div className="flex flex-col">
                <span className="text-xs lg:text-md">{ convertToDDMMYY(cardetails?.end_date)||"NA"}</span>
